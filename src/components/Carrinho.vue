<template>
  <div class="container wrapper">
    <h6 class="text-sem-produtos" v-if="!produtos.length">Você ainda não possui produtos no carrinho.</h6>
      ...
      <tr v-for="(produto, index) in produtos" v-bind:key="produto.name">
        <td data-title="PRODUTOS">
          ...
          <div class="inline-flex">
            <img v-on:click="removeProdutoCarrinho(index, produto)" class="icon-trash" src="../assets/icons/garbage.svg">
    ...
    <div v-if="produtos.length" id="row-totais" class="row pt-4 pb-4">
      ...
      <div class="col-md-2">
        <span class="text-total text-parcelado">em até <strong>10x R${{ getTotalParcelado() }}</strong>
        <br>
        (Total R${{ totalVista }})
        </span>
      </div>
    ...
    <div v-if="produtos.length" class="row mt-5">
      ...
      <div id="col-botoes-cart" class="col-md-9 inline-flex">
        <router-link to="/home">
          <button id="btn-continuar-comprando" class="btn-cart">
            Continuar comprando
          </button>
        </router-link>

        <router-link to="/checkout">
          <button id="btn-concluir-compra" class="btn-cart ml-4">
            Concluir compra
          </button>
        </router-link>
...
export default {
  ...
  methods: {
    ...
    getProdutosCarrinho: function () {
      if (localStorage.produtos) {
        ...
        for (let i = 0; i < this.produtos.length; i++) {
          this.produtos[i].quantidade = this.getQuantidadesProduto(this.produtos, this.produtos[i].name)
        }
    ...
    removeProdutoCarrinho: function (indexProduto, produto) {
      ...
      this.$root.$emit('REMOVED_ITEM_CART', this.produtos[indexProduto].quantidade)
...
<style lang="scss">
  ...
  .icon-trash {
    width: 15px;
    filter: invert(99%) sepia(1%) saturate(971%) hue-rotate(316deg) brightness(96%) contrast(69%);
    cursor: pointer;
  }
